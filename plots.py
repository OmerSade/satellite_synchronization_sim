...
def _orbit_convergence_from_satellites(
    ...
    then satellites belonging to the same orbit are averaged into a single orbit
    convergence trace.
    """
    ...
    referenced_errors = errors - np.nanmean(errors, axis=1, keepdims=True)
...
def _plot_orbit_waterfall_lines(
    ...
    ax.set_zlabel("Time error vs. 72-satellite average [s]", labelpad=8)
